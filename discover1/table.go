...
func newTable(t transport, ourID NodeID, ourAddr *net.UDPAddr, nodeDBPath string) (*Table, error) {
	...
	db := NewNodeDB(nodeDBPath, Version, ourID)
	tab := &Table{
		net:        t,
		db:         db,
		self:       NewNode(ourID, ourAddr.IP, uint16(ourAddr.Port)),
...
func (tab *Table) bond(pinged bool, id NodeID, addr *net.UDPAddr) (*Node, error) {
	if id == tab.self.ID {
...
func (tab *Table) pingpong(w *bondproc, pinged bool, id NodeID, addr *net.UDPAddr) {
	...
	w.n = NewNode(id, addr.IP, uint16(addr.Port))

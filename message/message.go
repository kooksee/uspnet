...
import (
	...
	"errors"
	"fmt"
	"github.com/kooksee/uspnet/proto"
)
...
type msgHdr struct {
	Magic    uint32
	CMD      [proto.MSG_CMD_LEN]byte // The message type
	Length   uint32
	Checksum [proto.CHECKSUM_LEN]byte
}
...
func AllocMsg(t string, length int) Messager {
	...
	case "inv":
		...
		msg.P.Blk = make([]byte, length-proto.MSG_HDR_LEN-1)
		return &msg
	case "getdata":
		var msg dataReq
		copy(msg.msgHdr.CMD[0:len(t)], t)
...
func MsgType(buf []byte) (string, error) {
	cmd := buf[proto.CMD_OFFSET : proto.CMD_OFFSET+proto.MSG_CMD_LEN]
	n := bytes.IndexByte(cmd, 0)
	if n < 0 || n >= proto.MSG_CMD_LEN {
...
func HandleNodeMsg(node proto.Noder, buf []byte, len int) error {
	...
	msg.Verify(buf[proto.MSG_HDR_LEN:len])

	return msg.Handle(node)
}

func magicVerify(magic uint32) bool {
	if magic != proto.NET_MAGIC {
		return false
	}
	return true
}
...
func LocateMsgHdr(buf []byte) []byte {
	var h msgHdr
	for i := 0; i <= len(buf)-proto.MSG_HDR_LEN; i++ {
...
func (hdr *msgHdr) init(cmd string, checksum []byte, length uint32) {
	hdr.Magic = proto.NET_MAGIC
	copy(hdr.CMD[0:uint32(len(cmd))], cmd)
	copy(hdr.Checksum[:], checksum[:proto.CHECKSUM_LEN])
	hdr.Length = length
}
...
func (msg *msgHdr) Deserialization(p []byte) error {
	buf := bytes.NewBuffer(p[0:proto.MSG_HDR_LEN])
	return binary.Read(buf, binary.LittleEndian, msg)
}

func (msg msgHdr) Serialization() ([]byte, error) {
	var buf bytes.Buffer
	return buf.Bytes(), binary.Write(&buf, binary.LittleEndian, msg)
}

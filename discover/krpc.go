...
func NewKRpc(priV *ecdsa.PrivateKey, lAddr string, nodeDBPath string, netRestrict *netutil.Netlist) (*Table, error) {
	...
	tab, _, err := newKRpc(priV, conn, nodeDBPath, netRestrict)
	if err != nil {
		return nil, err
	}
...
func newKRpc(priV *ecdsa.PrivateKey, c conn, nodeDBPath string, netRestrict *netutil.Netlist) (*Table, *udp, error) {
	udp := &KRpc{
		conn:        c,
		priV:        priV,
...
func (t *KRpc) WaitPing(from NodeID) error {
...
func (t *KRpc) pending(id NodeID, pt byte, callback func(interface{}) bool) <-chan error {
...
func (t *KRpc) handleReply(from NodeID, pt byte, req Packet) bool {
...
func (t *KRpc) Send(toAddr *net.UDPAddr, req Packet) error {
...
func (t *KRpc) handlePacket(from *net.UDPAddr, buf []byte) error {
...
func (t *KRpc) broadcast(p broadcast) {
	//t.ReadRandomNodes()
	//t.send()
}
func (t *KRpc) unicast(p unicast) {
	//t.send()
}
func (t *KRpc) multicast(p multicast) {
	//t.send()
}
